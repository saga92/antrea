...
./sonobuoy run --wait --e2e-focus='\[Feature:NetworkPolicy\]' --e2e-skip='allow-all|default-deny|named'
results=$(./sonobuoy retrieve)
echo 'Print all results'
./sonobuoy results $results
echo 'Print failed cases'
$ sonobuoy results $tarball --mode=detailed | jq 'select(.status=="failed")'
./sonobuoy delete --wait
