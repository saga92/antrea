...
func (n *NetworkPolicyController) labelsMatchGroupSelector(podLabels map[string]string, podNS *v1.Namespace, sel antreatypes.GroupSelector) bool {
	if sel.Namespace != "" {
		if sel.Namespace != podNS.Name {
			// Pods must be matched within the same Namespace.
			klog.V(4).Infof("Resource Namespace %s does not match selector Namespace %s", podNS.Name, sel.Namespace)
			return false
		}
		...
		klog.V(4).Infof("Pod labels %v match PodSelector", podLabels)
		return true
		...
		if !nSelector.Matches(labels.Set(podNS.Labels)) {
		...
		if !pSelector.Matches(labels.Set(podLabels)) {
